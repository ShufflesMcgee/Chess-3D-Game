...
public class ReallyBadChessControl : MonoBehaviour 
{
	public static ReallyBadChessControl Instance {set; get;}
...
/*	private void mouseOverSquare()
	...
	{
		int [] newTeam = generateNewTeam();
		for(int i = 0; i< newTeam.Length; i++)
		{
			if(newTeam[i] == 1)
				newTeam[i] = 5;//pawn
			if(newTeam[i] == 3)//bishop
				if(Random.value < .5f)
					newTeam[i] = 4;//knight
			if(newTeam[i] == 4)
				newTeam[i] = 2;//rook
		...
		spawnChessMen(newTeam[0], 0, 0, whiteFacingDirection);
		spawnChessMen(newTeam[1], 1, 0, whiteFacingDirection);
		spawnChessMen(newTeam[2], 2, 0, whiteFacingDirection);
		spawnChessMen(newTeam[3], 3, 0, whiteFacingDirection);
		spawnChessMen(0, 4, 0, whiteFacingDirection);//king
		spawnChessMen(newTeam[4], 5, 0, whiteFacingDirection);
		spawnChessMen(newTeam[5], 6, 0, whiteFacingDirection);
		spawnChessMen(newTeam[6], 7, 0, whiteFacingDirection);
		spawnChessMen(newTeam[7], 0, 1, whiteFacingDirection);
		spawnChessMen(newTeam[8], 1, 1, whiteFacingDirection);
		spawnChessMen(newTeam[9], 2, 1, whiteFacingDirection);
		spawnChessMen(newTeam[10], 3, 1, whiteFacingDirection);
		spawnChessMen(newTeam[11], 4, 1, whiteFacingDirection);
		spawnChessMen(newTeam[12], 5, 1, whiteFacingDirection);
		spawnChessMen(newTeam[13], 6, 1, whiteFacingDirection);
		spawnChessMen(newTeam[14], 7, 1, whiteFacingDirection);
	}
	...
	{ 
		...
		spawnChessMen(newTeam[0], 0, 7, blackFacingDirection);
		spawnChessMen(newTeam[1], 1, 7, blackFacingDirection);
		spawnChessMen(newTeam[2], 2, 7, blackFacingDirection);
		spawnChessMen(newTeam[3], 3, 7, blackFacingDirection);
		spawnChessMen(6, 4, 7, blackFacingDirection);//king
		spawnChessMen(newTeam[4], 5, 7, blackFacingDirection);
		spawnChessMen(newTeam[5], 6, 7, blackFacingDirection);
		spawnChessMen(newTeam[6], 7, 7, blackFacingDirection);
		spawnChessMen(newTeam[7], 0, 6, blackFacingDirection);
		spawnChessMen(newTeam[8], 1, 6, blackFacingDirection);
		spawnChessMen(newTeam[9], 2, 6, blackFacingDirection);
		spawnChessMen(newTeam[10], 3, 6, blackFacingDirection);
		spawnChessMen(newTeam[11], 4, 6, blackFacingDirection);
		spawnChessMen(newTeam[12], 5, 6, blackFacingDirection);
		spawnChessMen(newTeam[13], 6, 6, blackFacingDirection);
		spawnChessMen(newTeam[14], 7, 6, blackFacingDirection);
	}

	private int getRand(int [] set)
	{
		return set[Random.Range(0, set.Length)];
	}

	private int [] generateNewTeam()
	{
		int count = 15;
		int [] newSet = new int [count];
		int [] withPawn = new int [] {1, 3, 3, 4, 5};
		int [] withOut = new int [] {3, 3, 4, 5};
		int total = 0;
		while(total != 50)
		{
			total = 0;
			...
			{
				if(i < 8)
					newSet [i] = getRand(withOut);
				else
					newSet [i] = getRand(withPawn);
				total += newSet [i];
			}
		}
		return newSet;
